...
export function AddProductModal({ open, loading, onClose, onSubmit }: AddProductModalProps): JSX.Element | null {
    ...
    <div className="tc-modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-5)' }}>
          <h2 className="tc-modal-title" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--brand-600)" strokeWidth="2.5">
              <path d="M12 5v14M5 12h14" />
            </svg>
            {es.inventory.addProduct}
          </h2>
          <button type="button" onClick={onClose} className="tc-btn tc-btn--ghost" style={{ width: 36, height: 36, padding: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
          <div className="tc-field" style={{ gridColumn: '1 / -1' }}>
            ...
            />
            {fieldError('code') && <span style={{ color: 'var(--danger-600)', fontSize: 'var(--text-xs)' }}>{fieldError('code')}</span>}
          </div>
          ...
          <div className="tc-field" style={{ gridColumn: '1 / -1' }}>
            ...
            />
            {fieldError('name') && <span style={{ color: 'var(--danger-600)', fontSize: 'var(--text-xs)' }}>{fieldError('name')}</span>}
          </div>
          ...
          <div className="tc-field">
            ...
            />
            {fieldError('price') && <span style={{ color: 'var(--danger-600)', fontSize: 'var(--text-xs)' }}>{fieldError('price')}</span>}
          </div>
          ...
          <div className="tc-field">
            ...
            <input
              ...
              onChange={(e) => setFormData((p) => ({ ...p, criticalStock: Number(e.target.value) }))}
              placeholder="2"
            />
          </div>
          ...
          <div className="tc-field">
            ...
            <select
              ...
              onChange={(e) => setFormData((p) => ({ ...p, taxRate: Number(e.target.value) / 100 }))}
            >
        ...
        {/* Actions */}
        <div className="tc-modal-actions" style={{ marginTop: 'var(--space-5)', paddingTop: 'var(--space-4)', borderTop: '1px solid var(--gray-100)' }}>
          <button type="button" onClick={onClose} className="tc-btn tc-btn--secondary" disabled={loading}>
            {es.common.cancel}
          </button>
          <button type="button" onClick={handleSubmit} className="tc-btn tc-btn--primary" disabled={loading}>
            {loading ? (
              <>
