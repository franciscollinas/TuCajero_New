...
export function registerBackupIpc(): void {
  ...
  ipcMain.handle(
    'backup:delete',
    async (_event, actorUserId: number, fileName: string): Promise<ApiResponse<{ success: boolean; message: string }>> => {
      try {
  ...
  ipcMain.handle(
    'backup:info',
    async (_event, actorUserId: number): Promise<ApiResponse<Awaited<ReturnType<typeof backupService.getBackupInfo>>>> => {
      try {
